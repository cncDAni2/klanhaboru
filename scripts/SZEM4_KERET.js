...
function vercheck(){try{
	if (typeof(CVERZIO)=="undefined") {naplo("Globál","Verzióellenõrzés nem lehetséges. <a href='https://raw.githubusercontent.com/cncDAni2/klanhaboru/main/szem4/SZEM4_VER.js' target='_BLANK'>Kattints ide</a> hogy megnézd mi a legfrisebb változat. A tiéd: "+VERZIO); return;}
...
function shorttest(){try{
	...
	if (nez[5].checked && parseInt(nez[4].value,10)==0) warn+="Kényszeríted a kémek küldését, de a küldendõ kém értékére 0 van megadva!\n";
	if (nez[6].checked && parseInt(nez[4].value,10)==0) warn+="Kezdõ kémtámadást szeretnél, de a küldendõ kém értékére 0 van megadva!\n";
	...
	if (nez[8].value=="") hiba+="A legkevesebb pihenõ idõ: 1 perc, ne hagyd üresen.\n";
	if (parseInt(nez[8].value,10)<1) hiba+="A legkevesebb pihenõ idõ: 1 perc.\n";
	if (parseInt(nez[8].value,10)>150) hiba+="150 percnél több pihenõ idõt nem lehet megadni.\n";
	if (nez[9].value=="") hiba+="A leggyorsabb ciklusidõ: 200 ms, ne hagyd üresen.\n";
	if (parseInt(nez[9].value,10)<200) hiba+="A leggyorsabb ciklusidõ: 200 ms\n";
	if (parseInt(nez[9].value,10)>3000) hiba+="3000 ms-nél több ciklusidõ felesleges, és feltûnõ. Írj be 3000 alatti értéket.\n";
	
	if (parseInt(nez[10].value,10)<20) hiba+="Raktár telítettségi értéke túl alacsony, így vélhetõleg sehonnan se fog fosztani.";
	
	if (hiba!="" && !FARM_PAUSE) szunet("farm",document.getElementsByName("farm")[0]);
	if (FARM_PAUSE) warn+="A farmoló jelenleg meg van állítva!";
	if (hiba!="") {alert2("<b>Egy vagy több beállítási hiba miatt nem indítható a farmoló! Javítsa, majd indítsa el a kiegészítõt.</b><br><br>"+hiba); return false;} 
...
function szunet(script,kep){try{
	switch (script){
		...
		case "vije":
			...
			break;
		case "idtamad":
			alert2("Ezt a script nem állítható meg, mivel nem igényel semmilyen erõforrást.<br>Ha a hangot szeretnéd kikapcsolni, megteheted azt a hangbeállításoknál.");
			break;
		case "epit":
...
/*Tipus: "szoveg" v "szam"*/
	...
	for (var i=1;i<prodtable.length;i++){
		switch (tipus) {
			...
			default: throw("Nem értelmezhetõ mi szerint kéne rendezni."); return;
		}
...
function add_farmolo(){ try{
	...
	if (dupla!="") alert2("Dupla falumegadások, esetleg nem lévõ saját faluk kiszûrve:\n"+dupla);
	return;	
}catch(e){alert(e);}}
function add_farmolando(){try{
	...
	if (dupla!="") alert2("Dupla falumegadások kiszûrve:\n"+dupla);
	return;	
}catch(e){alert(e);}}
...
function szem4_farmolo_updater(){try{
	...
	for (var i=1;i<a.length;i++){try{
		...
		if (isNaN(uj)) {debug("Farmoló","Autófrissítõ anomália: "+a[i].cells[0].innerHTML+"falunál: "+a[i].cells[3].innerHTML+"+"+nov+"=Nem szám!"); continue;}
	...
	}catch(e){debug("Farmoló","Hiba az autofrissítõnél: "+e);}}
	setTimeout("szem4_farmolo_updater()",60000);
	return;
}catch(e){debug("Farmoló","Hiba az autofrissítõnél: "+e);setTimeout("szem4_farmolo_updater()",60000);}}
...
function szem4_farmolo_1kereso(){try{/*Farm keresi párját :)*/
	...
	var par=new Array(); /*Távolság;Gyalogosok e?;Koordináta-honnan;koord-hova*/
...
function szem4_farmolo_3egyeztet(adatok){try{
	...
	if (adatok[1].indexOf("semmi")==-1 || adatok[6]>hatszam*2) {
		...
		if (adatok[0]>parseInt(opts[3].value)){
			...
			/*debug("farm3",ido+"p a táv, "+d+" az idõ - összenyomjuk!");*/
	...
	/*Legyen e 3. lépés;sárga hátteres idõ lesz?;honnan;---*/
...
function szem4_farmolo_motor(){try{
	...
	switch (FARM_LEPES){
		case 0: /*Meg kell nézni mi lesz a célpont, +nyitni a HONNAN-t.*/
				...
				FARM_LEPES=1;
				break;
		case 1: /*Gyül.helyen vagyunk, be kell illeszteni a megfelelõ sereget, -nyers.*/ 
				if (isPageLoaded(FARM_REF,koordTOid(PM1[2]),"screen=place")) {FARM_HIBA=0; FARM_GHIBA=0;
					PM1=szem4_farmolo_2illeszto(PM1); 
					if (typeof(PM1)=="object" && PM1.length>0) {/*debug("Farmoló_ToStep2",PM1);*/ if (PM1[1].indexOf("semmi")>0) FARM_LEPES=3; else FARM_LEPES=2;} else FARM_LEPES=0;
				} else {FARM_HIBA++;}
				break;
		case 2: /*Confirm: nem e jött piros szöveg, játékos e -> OK-ézás.*/ 
				...
				break;
		case 3: /*Támadás elküldve, idõt és ID-t nézünk, ha kell.*/ 
				/*Kell e idõt nézni? Kell, ha PM1[1].indexOf("semmi")>-1 VAGY PM1[0]=TRUE; */
...
function szem4_VIJE_1kivalaszt(){try{
	...
	for (var i=VT.length-2;i>0;i--){
		...
		/*debug("VIJE1()","Ezt nézem: koord "+koord+". Idõ: "+d+", most: "+d2+". Különbség: "+(d2-d));*/
...
function VIJE_adatbeir(koord,nyers,banya,fal,szin){try{
	...
	if (nyers!=="")	{farm_helye.cells[3].innerHTML=nyers; 
		/*DEBUG*/if (nyers>parseInt(document.getElementById("farm_opts").rows[2].cells[1].getElementsByTagName("input")[3].value,10)) debug("VIJE_adatbeír","Sok nyers van itt, lehet hiba? "+koord+" falunál");}
	...
	if (szin==="blue") {} else 
	{farm_helye.cells[0].style.backgroundColor="red"; naplo("Jelentés Elemzõ",koord+" farm veszélyesnek ítélve. Jelentésének színe "+szin+".");}
...
function szem4_VIJE_2elemzes(adatok){try{
	...
	if (VIJE_REF2.document.getElementById("attack_spy_resources")){
		...
		if (adatok[4]) {var nyersossz="";debug("VIJE2","Nem kell elemezni (régi)"); } else {
			...
			var nyersossz=0;
			for (var i=0;i<nyers.length;i++) nyersossz+=parseInt(nyers[i],10);}catch(e){var nyersossz=0; debug("VIJE","<a href='"+VIJE_REF2.document.location+"'>"+adatok[0]+"</a> ID-jû jelentés nem szokványos, talált nyers 0-ra állítva. Hiba: "+e);}
		}
...
function szem4_VIJE_motor(){try{
	...
	if (VIJE_HIBA>10) {
		...
		if(VIJE_GHIBA>3) {
			if (VIJE_GHIBA>5) {
				naplo("Globál","Nincs internet? Folyamatos hiba a jelentés elemzõnél"); nexttime=60000; playSound("bot2");
			} 
			VIJE_REF1.close();
		} VIJE_LEPES=0;
	}
	
	if (VIJE2_HIBA>10) {VIJE2_HIBA=0; VIJE2_GHIBA++; if(VIJE2_GHIBA>3) {if (VIJE2_GHIBA>5) naplo("Globál","Nincs internet? Folyamatos hiba a jelentés elemzõnél"); VIJE_REF2.close();} VIJE_LEPES=0;}
...
ujkieg_hang("Bejövõ támadások","bejovo");
ujkieg("idtamad","Bejövõ támadások",'<tr><td align="center" ><table class="vis" id="idtamad_Bejovok" style="vertical-align:top; display: inline-block;"><tr><th>Idõpont</th><th>Támadások száma</th></tr></table> </td></tr>');
...
$(document).ready(function(){
	...
	soundVolume(0.2);
	playSound("bot2");
	setTimeout(function(){naplo("Indulás","Hangteszt vége. Ha nem hallotta a sípszót, vélhetõleg nem elérhetõ a cnc weboldal - használja az alternatív botriadót!"); soundVolume(1.0);},3000);
...
FIXME: Számolja bele az odaérkezés alatt keletkezõ nyerseket, és úgy keresse a "párját"
...
TRANSFORM: Gombbal állítódjon át a cucc (legyen RESET is), ezt olvassa ki, ekkor ellenõrizzen.
